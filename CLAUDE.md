...
Hard rules:
- `eventstore-core` and every `eventstore-<provider>` module must have zero dependency on any specific framework (Axon or otherwise) — see @docs/ARCHITECTURE.md. Framework coupling only lives in `integrations/eventstore-<framework>`.
